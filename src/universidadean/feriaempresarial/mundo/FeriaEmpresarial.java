...
public class FeriaEmpresarial {
    ...
    public String metodo1() {
        ...
        String respuesta1 = null;
            ...
            for (Puesto puesto : puestos) {
                if (puesto.darZona() == "Zona norte") {
                    for (Empresa empresa : empresas) {
                        if (puesto.darNombreEmpresa() == empresa.darNombre()) {
                            numeroNorte += empresa.darNumeroPersonasAsistentes();
                        }
                    }
                }
            }
            for (Puesto puesto : puestos) {
                if (puesto.darZona() == "Zona oriente") {
                    for (Empresa empresa : empresas) {
                        if (puesto.darNombreEmpresa() == empresa.darNombre()) {
                            numeroOriente += empresa.darNumeroPersonasAsistentes();
                        }
                    }
                }
            }
            for (Puesto puesto : puestos) {
                if (puesto.darZona() == "Zona occidente") {
                    for (Empresa empresa : empresas) {
                        if (puesto.darNombreEmpresa() == empresa.darNombre()) {
                            numeroOccidente += empresa.darNumeroPersonasAsistentes();
                        }
                    }
                }
            }
            for (Puesto puesto : puestos) {
                if (puesto.darZona() == "Zona centro") {
                    for (Empresa empresa : empresas) {
                        if (puesto.darNombreEmpresa() == empresa.darNombre()) {
                            numeroCentro += empresa.darNumeroPersonasAsistentes();
                        }
                    }
                }
            }
            for (Puesto puesto : puestos) {
                if (puesto.darZona() == "Zona sur") {
                    for (Empresa empresa : empresas) {
                        if (puesto.darNombreEmpresa() == empresa.darNombre()) {
                            numeroSur += empresa.darNumeroPersonasAsistentes();
                        }
                    }
                }
            }
            ...
            if (numeroOriente >= numeroNorte && numeroOriente >= numeroOccidente && numeroOriente >= numeroCentro && numeroOriente >= numeroSur) {
                respuesta1 = "La zona con mayor número de expositores es la oriente";
            }
            if (numeroOccidente >= numeroNorte && numeroOccidente >= numeroOriente && numeroOccidente >= numeroCentro && numeroOccidente >= numeroSur) {
                respuesta1 = "La zona con mayor número de expositores es la occidente";
            }
            if (numeroCentro >= numeroNorte && numeroCentro >= numeroOccidente && numeroCentro >= numeroOriente && numeroCentro >= numeroSur) {
                respuesta1 = "La zona con mayor número de expositores es la centro";
            }
    ...
    public String metodo2() {
        ...
        int nMaxExpositores = MAX_PERSONAS_NORTE*NUM_PUESTOS_NORTE + MAX_PERSONAS_ORIENTE*NUM_PUESTOS_ORIENTE + MAX_PERSONAS_OCCIDENTE*NUM_PUESTOS_OCCIDENTE + MAX_PERSONAS_SUR*NUM_PUESTOS_SUR + MAX_PERSONAS_CENTRO*NUM_PUESTOS_CENTRO;
        int nExpositores = 0;
        ...
        for (Puesto puesto:puestos){
            for (Empresa empresa:empresas){
                if (puesto.darNombreEmpresa() == empresa.darNombre()){
                    nExpositores += empresa.darNumeroPersonasAsistentes();
                }
            }
        }

        porcentajeExpositores = (double) nExpositores / nMaxExpositores;
